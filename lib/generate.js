...
const configUtils = require('sao/lib/utils/config');
const handlePrompt = require('sao/lib/utils/handle-prompt');
const SAOError = require('sao/lib/utils/sao-error');
const log = require('sao/lib/utils/log');
const utils = require('sao/lib/utils');
const promptStore = require('sao/lib/utils/prompt-store');
...
module.exports = co.wrap(function*({
	...
	const projectConfig = yield configUtils.getConfig(fromPath, configFileName, configOptions);
	...
	const showTip = () => {
		...
		if (isNewFolder) {
			log.info(
				`Type \`cd ${chalk.yellow(path.relative(process.cwd(), folderPath))}\` to get started!`
			);
		}
	};
	...
	const enforceType = projectConfig.enforceNewFolder
		? 'new'
		: projectConfig.enforceCurrentFolder
			? 'current'
			: null;
		...
		async data(answers) {
			const templateData =
				typeof projectConfig.data === 'function'
					? await projectConfig.data(answers)
					: projectConfig.data;
	...
	if (projectConfig.template === 'handlebars') {
		...
		kopyOptions.template = utils.requireAt(fromPath, `jstransformer-${projectConfig.template}`);
	} else {
