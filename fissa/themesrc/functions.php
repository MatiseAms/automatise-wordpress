...
switch (MATISE_ENVIRONMENT) {
	case 'local':
		define('API_DOMAIN', 'fissa.test');
		define('FRONTEND_DOMAIN', 'localhost:3000');
	break;
	default:
		define('API_DOMAIN', 'fissa.flywheelsites.com');
		define('FRONTEND_DOMAIN', 'fissa.matise.org');
	break;
}
...
function theme_scripts() {
	...
	wp_enqueue_style('theme-css', $include_url.'/assets/css/app.css', array(), date("is"), false);
	wp_enqueue_script( 'theme-appjs', get_theme_file_uri( '/assets/js/app.js' ), array(), date("is"), true );
}
